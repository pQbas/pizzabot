...
void rpm_left_wheel_callback( const std_msgs::Int16& cmd_msg){
  sp = cmd_msg.data;   
  reverse = false;
  if (sp<0){
    sp = -sp;
...
std_msgs::Int16 current_rpm_left_wheel;
ros::NodeHandle  nh;

ros::Subscriber<std_msgs::Int16> sub("rpm_left_wheel_tx", rpm_left_wheel_callback);
ros::Publisher pub("rpm_left_wheel_rx", &current_rpm_left_wheel);
...
ISR(TIMER1_COMPA_vect)
{
  ...
  if(reverse == true){
    ...
    current_rpm_left_wheel.data = -x_0;
    if (x_0 < 13){
      current_rpm_left_wheel.data = 0;
    }
  ...
  pub.publish(&current_rpm_left_wheel);
  nh.spinOnce();
