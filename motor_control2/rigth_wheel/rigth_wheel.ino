...
void rpm_wheel_callback( const std_msgs::Float32& cmd_msg){
  sp = cmd_msg.data;
  reverse = false;
  if (sp<0){
  //  sp = -sp;
...
std_msgs::Float32 current_rpm_wheel;
// std_msgs::Int16 PI_data;

ros::NodeHandle  nh;

ros::Subscriber<std_msgs::Float32> sub("rpm_right_wheel_tx", rpm_wheel_callback);
ros::Publisher pub("rpm_right_wheel_rx", &current_rpm_wheel);
// ros::Publisher pub_PI("PI_right_wheel_rx", &PI_data);
...
ISR(TIMER1_COMPA_vect)
{
  ...
  if (sp < VEL_MAX && sp > -VEL_MAX){
    
    digitalWrite(3,LOW);
    digitalWrite(5,LOW);
    //analogWrite(5,0.0);
    current_rpm_wheel.data = 0.0;

  }else if (sp < -VEL_MAX) {

      digitalWrite(3,LOW);
      analogWrite(5,-duty_cycle);
      current_rpm_wheel.data = x_0;  
  ...
  // pub_PI.publish(&PI_data);
  pub.publish(&current_rpm_wheel);
  nh.spinOnce();
